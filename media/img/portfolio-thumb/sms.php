...
class SMS extends MY_Controller
{
    ...
    * */
    ...
    public function subscribe($message)
    {
        ...
        $aggregator_code = $this->user->get_by('is_aggregator',$aggregator_code)->id;
        $aggregator_code = ($aggregator_code) ? $aggregator_code : null;
        ...
        if($account_exists)
        {
            ...
            if($aggregator_exists)
            {
                ...
                $reply = "Hi {$firstname}, You have updated your Account. Your Aggregator Code is {$aggregator_code}. For more call 0707933993.";
            }
            else
            {
                if($aggregator_code)
                {
                    ...
                    $reply = "Hi {$firstname}, You have updated your Account. The Aggregator Code {$aggregator_code} is NOT registered with us please call 0707933993 for help.";
                }
                else
                {
                    // $reply = lang('account_update', array($firstname));
                    $reply = "Hi {$firstname}, You have updated your Account. Please call 0707933993 for help.";
                }
            }
        }
        else
    ...
    public function incoming_sms()
    {
        ...
        $this->data['link'] = $this->router->class.'/'.$this->router->method;
        $this->data['texts'] = $this->incoming_text->filter($filter)->get_all();
    }

    public function price_texts()
    {
        $this->view = 'sms/incoming_sms';
        $filter = $this->uri->uri_to_assoc();
        $this->data['link'] = $this->router->class.'/'.$this->router->method;
    ...
    public function sms_order()
    {
        ...
        // Validation
        $this->form_validation->set_rules('buyer_contact', 'Contact', 'trim|required|min_length[9]|max_length[12]|xss_clean|numeric');
    ...
    private function _format_phone_number($phone_string)
    {
    ...
    private function curl_sms_action($mobile_number, $text, $redirect_url)
    {
        $this->view = false;
