...
class Ion_auth
{
	...
	//need to test email activation
	{
		...
		if (!$email_activation)
		{
			$id = $this->ci->ion_auth_model->register($username, $password, $email, $additional_data);
			if ($id !== FALSE)
			{
				$this->ci->session->set_flashdata('message','Invitation Creation was Successful');
				$this->ci->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful'));
				return $id;
			}
			else
		...
		else
		{
			...
			$this->ci->db->where('id',$id)->update('users', array('referral_id'=>$referrer));

			$activation_code = $this->ci->ion_auth_model->activation_code;
			$user            = $this->ci->ion_auth_model->user($id)->row();
			$identity        = $this->ci->config->item('identity', 'ion_auth');
			$referral        = $this->ci->ion_auth_model->user($referrer)->row();
			$data = array(
			...
			$message = $this->ci->load->view($this->ci->config->item('email_templates', 'ion_auth').$this->ci->config->item('email_invite', 'ion_auth'), $data, true);

			$this->ci->email->clear();
			$this->ci->email->set_newline("\r\n");
			$this->ci->email->from($this->ci->config->item('admin_email', 'ion_auth'), $this->ci->config->item('site_title', 'ion_auth'));
			$this->ci->email->to($email);
			$this->ci->email->subject($this->ci->config->item('site_title', 'ion_auth') . ' - Account Invitation');
			$this->ci->email->message($message);
	...
	{
		return $this->ci->ion_auth_model->user()->row();
	}
